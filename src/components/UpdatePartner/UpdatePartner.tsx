...
import {
  ...
  fetchPartnerById,
  updatePartner,
} from "../../app/features/partners/partnerSlice.ts";

export const UpdatePartner: React.FC = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();
  const { partnerDetail } = useSelector((state: RootState) => state.partner);
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      const resultAction = await dispatch(updatePartner({id: id, partner: formData}));
      if (updatePartner.fulfilled.match(resultAction)) {
  ...
  useEffect(() => {
    if (partnerDetail) {
      formik.setFieldValue("nameEn", partnerDetail.nameEn);
      formik.setFieldValue("nameAr", partnerDetail.nameAr);
      formik.setFieldValue("partnerUrl", partnerDetail.partnerUrl);
      formik.setFieldValue("shortName", partnerDetail.shortName);
      formik.setFieldValue("description", partnerDetail.description);
      formik.setFieldValue("prizePool", partnerDetail.prizePool);
      formik.setFieldValue("order", partnerDetail.order);
      formik.setFieldValue("partnerPic", partnerDetail.partnerPic);
      formik.setFieldValue("backgroundImage", partnerDetail.backgroundImage);
      formik.setFieldValue("partnerNews", partnerDetail.partnerNews);
      formik.setFieldValue("partnerSponsors", partnerDetail.partnerSponsors);
      formik.setFieldValue("websiteUrl", partnerDetail.websiteUrl);
      formik.setFieldValue("facebookUrl", partnerDetail.facebookUrl);
      formik.setFieldValue("twitterUrl", partnerDetail.twitterUrl);
      formik.setFieldValue("youtubeUrl", partnerDetail.youtubeUrl);
      formik.setFieldValue("twitchUrl", partnerDetail.twitchUrl);
      formik.setFieldValue("instagramUrl", partnerDetail.instagramUrl);
      formik.setFieldValue("discordUrl", partnerDetail.discordUrl);
    }
  }, [partnerDetail]);
