...
const UsersModel: React.FC<UsersModelProps> = ({ show, onClose, selectedUser, onSave }) => {
  const [profilePicPreview, setProfilePicPreview] = useState<string | undefined>(undefined);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      dateOfBirth: selectedUser?.dateOfBirth ? selectedUser.dateOfBirth.slice(0, 10) : "",
  ...
  useEffect(() => {
    if (selectedUser?.profilePicture) {
      if (typeof selectedUser.profilePicture === 'string') {
        setProfilePicPreview(baseURL + "/api/v1/" + selectedUser.profilePicture);
          ...
          <div className="p-4 md:p-5 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="relative float-label-input custom-input mb-4">
                <select
                  id="gender"
                  className={`w-full text-[0.94rem] text-white focus:outline-0 focus:!border focus:!border-highlight-color pt-[1.5rem] pb-[0.35rem] bg-input-color rounded-[0.52rem] px-3 block appearance-none leading-normal ${formik.values.gender ? 'pt-[1.5rem]' : 'pt-[0.35rem]'}`}
                ...
                <label
                  htmlFor="gender"
                  className={`absolute left-3 transition-all duration-200 text-[0.94rem] font-bold text-custom-gray pointer-events-none ${formik.values.gender ? 'top-1 -translate-y-0.5 font-normal text-[13px]' : 'top-1/2 -translate-y-1/2'}`}
                >
                  Gender
                </label>
              </div>
              ...
              <div className="relative float-label-input custom-input mb-4">
                <select
                  id="role"
                  className={`w-full text-[0.94rem] text-white focus:outline-0 focus:!border focus:!border-highlight-color pt-[1.5rem] pb-[0.35rem] bg-input-color rounded-[0.52rem] px-3 block appearance-none leading-normal ${formik.values.role ? 'pt-[1.5rem]' : 'pt-[0.35rem]'}`}
                ...
                <label
                  htmlFor="role"
                  className={`absolute left-3 transition-all duration-200 text-[0.94rem] font-bold text-custom-gray pointer-events-none ${formik.values.role ? 'top-1 -translate-y-0.5 font-normal text-[13px]' : 'top-1/2 -translate-y-1/2'}`}
                >
                  Role
                </label>
              </div>
              ...
              <div className="flex items-center gap-2 mb-4">
                <Switch
                  ...
                  id="isBanned"
                />
                <label htmlFor="isBanned" className="text-white">Is Banned</label>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
              {SOCIAL_FIELDS.map(({ key, label }) => (
                <div key={key} className="relative float-label-input custom-input mb-4">
                  <input
                    type="text"
                    id={key}
                    placeholder=" "
                    ...
                    onChange={e => formik.setFieldValue("socialMediaHandles", { ...formik.values.socialMediaHandles, [key]: e.target.value })}
                  />
                  <label
                    htmlFor={key}
                    className="absolute top-3 left-0 translate-y-[0.2rem] font-bold text-[0.94rem] pointer-events-none transition duration-200 bg-transparent px-3 text-custom-gray"
