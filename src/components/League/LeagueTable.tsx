        ...
        <tbody>
          {leagues.map((league, index) => (
            ...
            >
              ...
              <td className="text-center text-[1.0625rem] py-3">{league?.game?.name}</td>
              <td className="text-center text-[1.0625rem] py-3">{league.format}</td>
              ...
              <td className="text-[1.0625rem] py-3 flex space-x-3 justify-center">
                <button
                  onClick={() => setOpen(open === league._id ? null : league._id)}
                  style={{
                    background:
                ...
                {open === league?._id && (
                  <div className="absolute right-0 z-50 mt-10 w-56 origin-top-right rounded-xl bg-white shadow-xl ring-1 ring-black ring-opacity-5 dark:bg-gray-800 dark:ring-gray-700 transform transition-all duration-200 ease-out">
                    <div className="p-2">
                      {/* Edit Button */}
                      <button
                        onClick={() => navigate(`/${partnerId}/leagues/edit/${league._id}`, {
                          state: { league }
                        })}
                      ...
                      >
                        ...
                        <span className="font-medium group-hover:text-red-600 dark:group-hover:text-red-400">Delete</span>
                      </button>
                      ...
                      <div className="flex items-center justify-between px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-medium text-gray-700 dark:text-gray-200">
                            {league?.isHidden ? 'Show League' : 'Hide League'}
                          </span>
                        </div>
                        ...
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            ...
                            onChange={async () => {
                              await dispatch(toogleleague({
                                id: league._id,
                                isHidden: !league?.isHidden
                              }));
                              dispatch(fetchLeagues({
                                partnerId: partnerId, page: 1,
                                perPage: 0,
                                searchTerm: ""
                              }));
                            }}
